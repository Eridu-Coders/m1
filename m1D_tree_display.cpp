...
M1MidPlane::Interp* M1UI::TreeDisplay::addTestInterpRecur(
...
void M1UI::TreeDisplay::addTestInterp(M1Store::Item_lv2* p_root){
    ...
    M1MidPlane::Interp* l_auto_edge = addTestInterpRecur(p_root, 0, l_vb, l_alrady_seen);
    l_vb->addStretch();
...
void M1UI::TreeDisplay::gotoVertex(M1Store::Item_lv2* p_new_vertex){
    ...
    if(p_new_vertex == nullptr)
        addTestInterp(m_root);
    else
        addTestInterp(p_new_vertex);
    M1_FUNC_EXIT
}
