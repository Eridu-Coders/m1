        ...
        for l_chap in l_json_chapters:
            ...
            for l_verse_number in range(1, l_verse_count + 1):
                ...
                if os.path.exists(l_cache_file_iskcon):
                    ...
                    print(l_url_iskcon)
                    try:
                        l_page_iskcon = urllib.request.urlopen(l_url_iskcon)
                        # Decode the bytes into a string using UTF-8
                ...
                if l_sk_test != l_sk_iskcon_comp and not l_iskcon_unavailable:
                    ...
                    #                     l_ending = ''
                    #                     l_match_ending = re.search(r'।।\d+\.\d+।।', l_sk)
                    #                     if l_match_ending:
                    #                         l_ending = l_match_ending.group(0)
                    #
                    #                     l_sk = re.sub(r'\s+', ' ', re.sub(r'\|\|[^|]+\|\|$', '', l_sk_local).replace('|', '।')).strip() + l_ending
                    l_sk = l_sk_iskcon
