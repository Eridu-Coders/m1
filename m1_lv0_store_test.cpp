...
TEST(ItemTest, ValuesRoundTripFullVertex){
    ...
    for(int t=0; t<20; t++){
        ...
        std::uniform_int_distribution<int> l_distr_member(1, 14); // define the range for the choice of member
        ...
        M1Store::Item l_item(l_id, l_flags, l_type);
        ...
        for(int i = 0; i<30; i++){
            ...
            */
            ...
            EXPECT_NEAR(l_item.creationDate().currentMSecsSinceEpoch(), l_date_creation.currentMSecsSinceEpoch(), 20);
            EXPECT_NEAR(l_item.lastmodDate().currentMSecsSinceEpoch(), l_date_lastmod.currentMSecsSinceEpoch(), 20);
...
TEST(ItemTest, ValuesRoundTripFullEdge){
    ...
    for(int t=0; t<20; t++){
        ...
        std::random_device l_rd; // obtain a random number from hardware
        std::mt19937 l_gen(l_rd()); // seed the generator
        std::uniform_int_distribution<int> l_distr_member(1, 16); // define the range for the choice of member
        std::uniform_int_distribution<M1Store::ItemID> l_distr_id(0, 0xffffffffffffffff -1); // define the range for ItemIDs
        std::uniform_int_distribution<M1Store::FlagField> l_distr_flags(0, 0xffffffffffffffff); // define the range for flags
...
TEST(ItemTest, ValuesRoundTripSimpleEdge){
    ...
    for(int t=0; t<20; t++){
        ...
        std::random_device l_rd; // obtain a random number from hardware
        std::mt19937 l_gen(l_rd()); // seed the generator
        std::uniform_int_distribution<int> l_distr_member(1, 7); // define the range for the choice of member
        std::uniform_int_distribution<M1Store::ItemID> l_distr_id(0, 0xffffffffffffffff -1); // define the range for ItemIDs
        std::uniform_int_distribution<M1Store::FlagField> l_distr_flags(0, 0xffffffffffffffff); // define the range for flags
